...
import { GlobeAltIcon, ShareIcon, EllipsisHorizontalIcon } from "@heroicons/react/24/outline"
  ...
  return (
    <Card className="absolute left-36 -top-24 h-56 w-3/4 rounded-3xl flex flex-row items-center justify-between p-6">
      ...
      <div className=" flex flex-col w-3/5 gap-2">
        ...
        <p className="text-sm text-foreground/40">{data.attributes.biography}</p>
        <div className="flex items-center gap-2 text-sm">
          <GlobeAltIcon className="h-5 w-5" />
          <a href="#">{data.attributes.website}</a>
        </div>
      ...
      <div className="w-1/5 flex gap-1">
        ...
        <Button isIconOnly radius="full" variant="light" startContent={<ShareIcon className="h-5 w-5" />} />
        <Button isIconOnly radius="full" variant="light" startContent={<EllipsisHorizontalIcon className="h-5 w-5" />} />
      </div>
    </Card>
  )
}
